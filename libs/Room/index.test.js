...
describe("Room Methods", () => {
  beforeEach(() => {
    newRoom = new Room(4, 1234, "uniqueId");
  });
  ...
  it("should have the number of teams given in", () => {
    const expected = 2;
    const testRoom = new Room(expected, 1234, "imauniqueuid123");
  ...
  it("should add specific player to team", () => {
    ...
    const newRoom = new Room(1, 1234, "imauniqueuid123");
    newRoom.addPlayerToTeam(teamColors[0], name, id);
    expect(newRoom.teams[teamColors[0]][0]).toEqual({ id, name });
  });

  it("should return true if player in room", () => {
    const name = "ben";
    const id = "playerUnique";
    const newRoom = new Room(1, 1234, "imauniqueuid123");
    newRoom.addPlayerToTeam(teamColors[0], name, id);
    expect(newRoom.isPlayerInRoom(id)).toBe(true);
  });
  ...
  it("should add the total points in scores array to scores", () => {
    ...
    expect(newRoom.scores[team]).toEqual(expectedScore);
  });

  it("should add the total points in scores array to scores with one function", () => {
    const expectedScore = 100;
    const team = teamColors[0];
    newRoom.addToCurrentScore(team, expectedScore);
