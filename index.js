...
function onConnection(socket) {
    ...
    socket.on("testNav", () =>
        socket.emit("pageNavigation", "/hostrouter/123")
    );
    ...
    socket.on("abort", roomId =>
        catchError(socket, roomId, endGamePrematurely)
    );
    ...
    socket.on("deleteGameRoom", room =>
        catchError(socket, room, deleteGameRoom)
    );
    ...
    socket.on("submitTeamAnswer", data =>
...
function startGameLoop(socket, roomId) {
    ...
    const currentQuestion = testQuestions[rooms[roomId].questionNumber];
    const currentQuestionNumber = rooms[roomId].questionNumber + 1;
...
function checkAnswersForOrderRound(roomId, team) {
    ...
    console.log(
        "answer feedback in app",
        rooms[roomId].getAnswerFeedback(team)
    );
...
function updateHost(roomId) {
    const currentRoom = rooms[roomId];
    io.in(userIds[currentRoom.host].currentSocket).emit(
        "updateHostRoom",
        Utils.getRoomReadyForSending(rooms[roomId])
    );
}
